...
export function PlanActions({
  ...
  const runPillar = (topicId: string) => {
    ...
    startTransition(async () => {
      const result = await planPillar(projectCode, topicId);
      if (!result.ok) {
  ...
  const runFull = () => {
    ...
    setInfo(null);
    startTransition(async () => {
      const result = await planFullSite(projectCode);
      if (!result.ok) {
        setError(result.error);
      } else {
    ...
    <div className="flex flex-col items-end gap-2">
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={runFull}
          disabled={pending}
          title="Generate suggested pillars + child pages for ALL unfulfilled topics in this project at once. Each suggestion lands as a dimmed row you can adopt or dismiss."
