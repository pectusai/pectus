    ...
    <span className="flex items-center gap-1">
      {enabledLocales.map((locale) => {
        ...
        return (
          <a
            key={locale}
            href={`/projects/${projectCode}/apps/content-hub/pages/builder/${v.variant_id}`}
...
function NodeTitle({
  ...
  if (!node.has_page) {
    return (
      <Link
        href={`/projects/${projectCode}/apps/content-hub/pages/new?nodeId=${node.id}`}
        className={className}
  ...
  return (
    <Link
      href={`/projects/${projectCode}/apps/content-hub/pages/builder/${node.default_variant_id}`}
      className={className}
