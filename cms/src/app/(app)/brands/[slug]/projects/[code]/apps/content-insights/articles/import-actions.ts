...
import { touchFreshness } from "@/lib/project";
...
export async function importArticlesFromSitemap(
  ...
  const { data: ws, error: wsErr } = await supabase
    ...
    .single();
  if (wsErr || !ws) {
    return { ok: false, error: `Project lookup failed: ${wsErr?.message ?? "not found"}` };
  }
  ...
  revalidatePath(`/projects/${projectCode}/apps/content-insights/articles`);
