...
> {
  ...
  revalidatePath(
    `/projects/${args.projectCode}/apps/content-insights/pages/builder/${args.variantId}`,
  );
...
/* ------------------------------------------------------------------------- *
 * publishVariant — commits the variant to the user's content-insights repo as a
...
> {
  ...
  const { data: ws } = await supabase
    ...
    .single();
  if (!ws) return { ok: false, error: "Project not found." };
  if (!ws.content_insights_repo) {
    return {
      ok: false,
      error:
        "This project has no content_insights_repo set. Add it under Project Settings → Site URL before publishing.",
    };
  }
  ...
  const repoString = ws.content_insights_repo;
  const slashIdx = repoString.indexOf("/");
  if (slashIdx <= 0) {
    return {
      ok: false,
      error: `content_insights_repo must be in the form 'owner/repo'. Got '${repoString}'.`,
    };
  }
  ...
  try {
    const result = await commitPagePublish(
      { owner, repo, branch: ws.content_insights_branch ?? "main" },
      {
  ...
  revalidatePath(`/projects/${args.projectCode}/apps/content-insights/pages`);
...
export async function switchOrCreateVariantForLocale(args: {
  ...
  if (existing) {
    redirect(
      `/projects/${args.projectCode}/apps/content-insights/pages/builder/${existing.id}`,
    );
  }
  ...
  revalidatePath(`/projects/${args.projectCode}/apps/content-insights/pages`);
  redirect(
    `/projects/${args.projectCode}/apps/content-insights/pages/builder/${created.id}`,
  );
}
