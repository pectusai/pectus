...
export async function saveSiteUrl(formData: FormData) {
  ...
  const contentInsightsRepo =
    String(formData.get("content_insights_repo") ?? "").trim() || null;
  const contentInsightsBranch =
    String(formData.get("content_insights_branch") ?? "main").trim() || "main";
  ...
  await supabase
    ...
    .update({
      ...
      content_insights_repo: contentInsightsRepo,
      content_insights_branch: contentInsightsBranch,
    })
    .eq("id", project.id);

  if (intent === "republish") {
    await republishProject(supabase, project.id, code);
  }

  revalidatePath(`/projects/${code}/apps/content-insights/settings/site-url`);
  revalidatePath(`/projects/${code}/settings`);
  revalidatePath(`/projects/${code}/apps/content-insights/pages`);
}
...
async function republishProject(
  ...
  const { data: ws } = await supabase
    .from("projects")
    .select(
      "id, code, default_locale, enabled_locales, default_locale_skips_prefix, mount_slug, content_insights_repo, content_insights_branch",
    )
    .eq("id", projectId)
    .single();
  if (!ws || !ws.content_insights_repo) return;

  const slashIdx = ws.content_insights_repo.indexOf("/");
  if (slashIdx <= 0) return;
  const owner = ws.content_insights_repo.slice(0, slashIdx);
  const repo = ws.content_insights_repo.slice(slashIdx + 1);
  const branch = ws.content_insights_branch ?? "main";

  const { data: variants } = await supabase
    .from("page_variants")
    .select(
      "id, page_id, locale, slug, title, meta_description, blocks, last_published_path, status, page:pages!inner(template_id, purpose, site_plan_node_id, project_id)",
    )
