...
function TreeNodeRow({
  ...
  const onAdopt = () => {
    startTransition(async () => {
      await adoptSuggestedNode(projectCode, node.id);
    });
  };
  const onDismiss = () => {
    startTransition(async () => {
      await dismissSuggestedNode(projectCode, node.id);
    });
  };
  ...
  return (
    <li>
      ...
      >
        ...
        <NodeTitle node={node} projectCode={projectCode} />
    ...
    <span className="flex items-center gap-1">
      {enabledLocales.map((locale) => {
        ...
        return (
          <a
            key={locale}
            href={`/projects/${projectCode}/apps/content-insights/pages/builder/${v.variant_id}`}
...
function NodeTitle({
  ...
  if (!node.has_page) {
    return (
      <Link
        href={`/projects/${projectCode}/apps/content-insights/pages/new?nodeId=${node.id}`}
        className={className}
  ...
  return (
    <Link
      href={`/projects/${projectCode}/apps/content-insights/pages/builder/${node.default_variant_id}`}
      className={className}
