...
export default async function SiteUrlSettingsPage({
  ...
  if (!(await isAppActiveForProject(project.id, "content-insights"))) {
    return <ActivateAppPointer appName="content-insights" surface="Site URL" />;
  }
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <label className="block text-sm">
          ...
          <input
            name="mount_slug"
            defaultValue={project.mount_slug ?? "/"}
            placeholder="/"
        ...
        <label className="block text-sm">
          ...
          <input
            name="default_locale"
            defaultValue={project.default_locale ?? "en"}
            placeholder="en"
        ...
        <label className="block text-sm">
          <span
            className="mb-1 block text-xs font-medium text-zinc-600"
            title="Comma-separated locales the project publishes in. Each enabled locale gets its own page tree variants and its own site-plan-<locale>.json file."
          >
            Enabled locales (comma-separated)
          </span>
          <input
            name="enabled_locales"
            defaultValue={(project.enabled_locales ?? ["en"]).join(", ")}
            placeholder="en, sv"
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            defaultChecked={project.default_locale_skips_prefix ?? true}
        ...
        <div className="border-t border-zinc-100 pt-5">
          ...
          <label className="block text-sm">
            ...
            <input
              name="content_insights_repo"
              defaultValue={project.content_insights_repo ?? ""}
          ...
          <label className="mt-3 block text-sm">
            ...
            <input
              name="content_insights_branch"
              defaultValue={project.content_insights_branch ?? "main"}
              placeholder="main"
              className="w-32 rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm font-mono"
            />
          </label>
      ...
      <p className="text-sm text-zinc-600">
        ...
        <Link
          href={`/projects/${code}/apps/content-insights/settings/redirects`}
          className="underline"
        >
          Redirects
        </Link>{" "}
