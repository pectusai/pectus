    ...
    <div className="mx-auto max-w-5xl px-6 py-10">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          One project per market or property under {brand.name ?? brand.slug}.
          Pick one to manage its ICP, keywords, and apps.
        </p>
      </div>

      {!projects || projects.length === 0 ? (
        <div className="rounded-lg border border-dashed border-zinc-300 p-10 text-center text-sm text-zinc-500">
          No projects yet. Create your first from the CLI:
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {(projects as Project[]).map((p) => {
            const freshness = freshnessByProject.get(p.id) ?? {};
            return (
              <li key={p.id}>
                <Link
                  href={`/brands/${slug}/projects/${p.code}`}
