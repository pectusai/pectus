...
> {
  ...
  const { data: page } = await supabase
    .from("pages")
    .select("template_id, purpose, workspace_id")
    .eq("id", variant.page_id)
    .single();
  if (!page || page.workspace_id !== ws.id) {
    return { ok: false, error: "Page not in workspace." };
  }
  ...
  revalidatePath(
    `/workspaces/${args.workspaceCode}/pages/builder/${args.variantId}`,
  );
...
> {
  ...
  const { data: page } = await supabase
    .from("pages")
    .select("id, workspace_id, template_id, purpose, site_plan_node_id")
    .eq("id", variant.page_id)
    .single();
  if (!page || page.workspace_id !== ws.id) {
    return { ok: false, error: "Page not in workspace." };
  ...
  /* Collect all adopted nodes for the workspace — used both for the
   * ancestor walk and for the per-locale site-plan JSON. */
  const { data: nodesRaw } = await supabase
    .from("site_plan_nodes")
    .select("id, parent_id, workspace_id, title, position, materialized_path, status")
  ...
  if (
     ...
     * rows for the workspace with the collapsed list. */
  ...
  const { data: variantsForPlan } = await supabase
    .from("page_variants")
    .select(
      "slug, locale, status, page:pages!inner(site_plan_node_id, workspace_id)",
    )
    .eq("locale", variant.locale)
  ...
  revalidatePath(`/workspaces/${args.workspaceCode}/pages`);
...
export async function switchOrCreateVariantForLocale(args: {
  ...
  if (existing) {
    redirect(
      `/workspaces/${args.workspaceCode}/pages/builder/${existing.id}`,
    );
  }
  ...
  revalidatePath(`/workspaces/${args.workspaceCode}/pages`);
  redirect(
    `/workspaces/${args.workspaceCode}/pages/builder/${created.id}`,
  );
}
