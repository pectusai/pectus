...
export default async function ReviewsPage() {
  ...
  /* Fetch open queue items joined with their workspace + the workspace's
   * review policy. Pectus filters on the client side to keep the SQL
   * straightforward; the volume here is small. */
  const { data: items } = await supabase
    .from("review_queue_items")
    .select(
      "id, workspace_id, subject, summary, state, required_roles, created_at, workspace:workspaces(code, name)",
    )
      ...
      <div className="mb-6 flex flex-wrap items-baseline justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Items waiting on your sign-off across every workspace.
          </p>
        </div>
            ...
            >
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                ...
                <span className="text-xs text-zinc-500">
                  {item.workspace?.name ?? "—"} · {item.workspace?.code ?? "—"}
                </span>
              </div>
              {item.summary ? (
