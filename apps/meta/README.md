...
Plus per-workspace `meta_ad_account_id` (the `act_<id>` form) in the `integrations` table.

## Setup

```
npx pectus connect meta --workspace <code>
```

## Schedule

```
npx pectus app fetch meta --workspace <code> --since <YYYY-MM-DD> --until <YYYY-MM-DD>
```
