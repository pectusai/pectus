...
- `google_ads_customer_id` — the customer (advertiser) account whose data this workspace pulls.
...
`ad_metrics`, with `channel = 'google_ads'`. Columns: `workspace_id`, `date`, `channel`, `level` (campaign/ad_group/keyword), `entity_id`, `entity_name`, `metric_name`, `value`, `dimensions` (JSONB).
...
```
npx pectus app fetch google-ads --workspace <code> --since 2026-04-01 --until 2026-04-30
```
