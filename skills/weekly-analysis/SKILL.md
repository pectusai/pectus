...
1. **Top 200 keywords** for this project — `{keyword, search_volume, current_rank, intent, gsc_impressions, gsc_clicks, gsc_position}`. Last 28 days. (Will be retired once `apps/gsc/insights/` ships in v0.3.)
...
6. **AnswerThePublic entries** (if the project has any) — questions the audience is asking, grouped by source. (Will be retired once `apps/answer-public/insights/` ships in v0.3.)
...
- **new_category_suggestions** — Topic areas the project doesn't cover but the data suggests it should.
...
- Don't recommend writing about something the project already has multiple articles on unless `old_posts_rising` flags it.
